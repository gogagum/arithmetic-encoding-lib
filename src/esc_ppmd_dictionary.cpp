...
auto PPMDDictionary::getWordOrd(const Count cumulativeCnt) const -> Ord {
  ...
  if (getEscDecoded_() <= currCtx.size()) {
    const CtxCell_& cell = getCurrCtxCell_(currCtx);
    return *upper_bound(
...
auto PPMDDictionary::getProbabilityStats(const Ord ord) -> StatsSeq {
  ...
  for (; !currCtx.empty(); currCtx.pop_back()) {
    CtxCell_& currCtxInfo = ctxInfo_.at(currCtx);
    if (currCtxInfo.cnt.getCount(ord) > 0) {
      break;
    }
    const Count currTotal = currCtxInfo.cnt.getTotalWordsCnt();
    const Count currTotalUnique = currCtxInfo.uniqueCnt.getTotalWordsCnt();
    const Count escLow = (2 * currTotal) - currTotalUnique;
    const Count escHigh = escLow + currTotalUnique;
    const Count escTotal = 2 * currTotal;
    ret.emplace_back(escLow, escHigh, escTotal);
    currCtxInfo.cnt.increaseOrdCount(ord, 1);
    currCtxInfo.uniqueCnt.update(ord);
  }
  if (currCtx.empty()) {
    const Count zeroTotal = zeroCtxCell_.cnt.getTotalWordsCnt();
    const Count zeroLowerUnique =
        zeroCtxCell_.uniqueCnt.getLowerCumulativeCnt(ord);
    if (const Count zeroCount = zeroCtxCell_.cnt.getCount(ord);
        0 == zeroCount) {
      const Count zeroTotalUnique = zeroCtxCell_.uniqueCnt.getTotalWordsCnt();
      const Count escLow = (2 * zeroTotal) - zeroTotalUnique;
      const Count escHigh = escLow + std::max(Count{1}, zeroTotalUnique);
      const Count escTotal = std::max(Count{1}, 2 * zeroTotal);
      ret.emplace_back(escLow, escHigh, escTotal);
      const Count symLow = Count{ord} - zeroLowerUnique;
      const Count symHigh = symLow + 1;
      const Count symTotal = getMaxOrd_() - zeroTotalUnique;
      ret.emplace_back(symLow, symHigh, symTotal);
    } else {
      const Count zeroLower = zeroCtxCell_.cnt.getLowerCumulativeCnt(ord);
      const Count zeroUnique = zeroCtxCell_.uniqueCnt.getCount(ord);
      const Count symLow = (2 * zeroLower) - zeroLowerUnique;
      const Count symHigh = symLow + (2 * zeroCount) - zeroUnique;
      const Count symTotal = 2 * zeroTotal;
      ret.emplace_back(symLow, symHigh, symTotal);
    }
  } else {
    const CtxCell_& currCtxInfo = ctxInfo_.at(currCtx);
    const Count lowerCnt = currCtxInfo.cnt.getLowerCumulativeCnt(ord);
    const Count lowerUniqueCnt =
        currCtxInfo.uniqueCnt.getLowerCumulativeCnt(ord);
    const Count symLow = (2 * lowerCnt) - lowerUniqueCnt;
    const Count cnt = currCtxInfo.cnt.getCount(ord);
    const Count uniqueCnt = currCtxInfo.uniqueCnt.getCount(ord);
    const Count symHigh = symLow + (2 * cnt) - uniqueCnt;
    const Count symTotal = 2 * currCtxInfo.cnt.getTotalWordsCnt();
    ret.emplace_back(symLow, symHigh, symTotal);
    for (; !currCtx.empty(); currCtx.pop_back()) {
      CtxCell_& currCtxInfo = ctxInfo_.at(currCtx);
      currCtxInfo.cnt.increaseOrdCount(ord, 1);
...
auto PPMDDictionary::getZeroCtxEscStats_() const -> ProbabilityStats {
  ...
  return {
      .low = escLow,
      .high = escLow + zeroCtxCell_.uniqueCnt.getTotalWordsCnt(),
      .total = 2 * zeroCtxCell_.cnt.getTotalWordsCnt(),
  };
}

...
TEST_P(NumericalEncodeDecode, EncodeDecodeAndCompare) {
  ...
  auto countsMapping = ael::NumericalCoder::countWords(params.encoded);

  const auto encodeRes =
      ael::NumericalCoder().encode(params.encoded, countsMapping);

  EXPECT_EQ(encodeRes.contentWordsEncoded, params.encoded.size());

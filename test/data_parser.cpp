...
TEST(DataParser, Construct) {
  const auto testData =
      std::array{std::byte{2}, std::byte{25}, std::byte{17}, std::byte{11}};
  [[maybe_unused]] auto decoded = DataParser(testData);
}

TEST(DataParser, NumBytes) {
  const auto testData = std::array{static_cast<std::byte>(std::uint8_t{2}),
                                   std::byte{25}, std::byte{17}, std::byte{11}};
...
TEST(DataParser, UInt16T) {
  ...
  EXPECT_EQ(decoded.takeT<std::uint16_t>(), std::uint16_t{0b0010011001100010});
}
